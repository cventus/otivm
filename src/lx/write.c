...
void print_stack_tags(struct wstate *w)
{
	struct lxvalue ref, val;
	int tag;

	ref = w->stack;
	while (!ref_eq(w->mem->alloc.tag_free, ref)) {
		val = lx_car(ref_to_list(ref));
		tag = val.tag;
		switch (tag) {
		case lx_list_tag: fprintf(stderr, "list(%p)\n", (void*)ref_cell(val)); break;
		case lx_tree_tag: fprintf(stderr, "tree(%p)\n", (void*)ref_cell(val)); break;
