...
{
	return lxtag_cdr(*ref_tag(list.value));
}

static inline enum lx_tag list_car_tag(struct lxlist list)
{
	return lxtag_tag(*ref_tag(list.value));
}

static inline union lxcell const *list_car(struct lxlist list)
{
	return ref_data(list.value);
}

static inline struct lxlist ref_to_list(struct lxvalue ref)
{
	return (struct lxlist) { .value = ref };
}
