...
/* Shrink buffer by `size` bytes from the end of the buffer and re-allocate the
   buffer if the occupancy is now too small. Return non-zero if buffer `size`
   is greater than the size of the buffer or if reallocation fails. */
